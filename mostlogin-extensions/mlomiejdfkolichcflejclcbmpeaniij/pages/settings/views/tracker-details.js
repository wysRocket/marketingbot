...
import { getStatus, toggleDomain as toggleDomain$1, toggleGlobal as toggleGlobal$1 } from "../../../utils/exceptions.js";
import tracker_add_exception_default from "./tracker-add-exception.js";
import website_details_default from "./website-details.js";
//#region src/pages/settings/views/tracker-details.js
/**
...
var tracker_details_default = {
	[router_default.connect]: { stack: () => [tracker_add_exception_default] },
	options: store_default(Options),
	tracker: store_default(Tracker),
	otherTrackers: ({ tracker }) => store_default.ready(tracker) && store_default.get([Tracker], { tracker: tracker.id }),
	domains: ({ options, tracker }) => store_default.ready(options, tracker) && options.exceptions[tracker.id]?.domains || [],
	exceptionStatus: ({ options, tracker }) => store_default.ready(options, tracker) && getStatus(options, tracker.id),
	render: ({ options, tracker, otherTrackers, domains, exceptionStatus }) => html`
    <template layout="contents">
      <settings-page-layout>
        <div layout="column gap">
          <settings-link href="${router_default.backUrl()}" data-qa="button:back">
            <ui-icon name="chevron-left" color="primary"></ui-icon>
            <ui-text type="headline-s" layout="row gap items:center"> Back </ui-text>
          </settings-link>
          ${store_default.ready(options, tracker) && html`
              ...
              <div layout="column gap:2">
                <div layout="row gap items:center content:space-between">
                  ...
                  <ui-button disabled="${exceptionStatus.trusted && exceptionStatus.global}">
                    <a href="${router_default.url(tracker_add_exception_default, { tracker })}"> Add </a>
                  </ui-button>
                </div>
                <settings-table disabled="${exceptionStatus.trusted && exceptionStatus.global}">
                  ...
                  ${domains.map((domain) => html`
                      <ui-action layout="block">
                        <a
                          href="${router_default.url(website_details_default, { domain })}"
              ...
              <div layout="column gap:4">
                ${store_default.ready(tracker.organization) && html`
                  <div layout="column gap:4" layout@768px="grid:2fr|1fr">
                    ${tracker.organization.name && html`
                      <div layout="column gap">
                  ...
                  ${tracker.organization.country && html`
                    <div layout="column gap">
                  ...
                  `}
                  ${tracker.organization.websiteUrl && html` <div layout="column gap">
                  ...
                  ${tracker.organization.privacyPolicyUrl && html`
                    <div layout="column gap">

...
import website_clear_cookies_default from "./website-clear-cookies.js";
//#region src/pages/settings/views/website-details.js
/**
...
function revokePaused({ options, domain }) {
	store_default.set(options, { paused: { [domain]: null } });
}
...
async function saveElementPickerSelectors(host, event) {
	...
	await store_default.set(host.elementPickerSelectors, { hostnames: { [host.domain]: selectors.length > 0 ? selectors : null } });
...
var website_details_default = {
	[router_default.connect]: {
		stack: () => [tracker_details_default, website_clear_cookies_default],
		replace: true
	},
	domain: "",
	...
	paused: ({ options, domain }) => store_default.ready(options) && options.paused[domain] || {},
	issueUrl: ({ config, domain }) => store_default.ready(config) && config.domains[domain]?.issueUrl,
	trackers: ({ options, domain }) => store_default.ready(options) ? Object.entries(options.exceptions).filter(([, { domains }]) => domains.includes(domain)).map(([id]) => id).sort((a, b) => a.localeCompare(b)).map((id) => store_default.get(Tracker, id)).map((tracker) => store_default.error(tracker) ? {
		id: tracker.id,
		name: tracker.id
	} : tracker) : [],
	elementPickerSelectors: store_default(ElementPickerSelectors),
	selectors: ({ elementPickerSelectors, domain }) => store_default.ready(elementPickerSelectors) && elementPickerSelectors.hostnames[domain]?.join("\n") || "",
	clearedCookies: false,
	render: ({ domain, topLevelDomain, paused, issueUrl, trackers, selectors, clearedCookies }) => html`
      ...
      <settings-page-layout layout="gap:4">
        ...
        >
          ...
          <settings-table>
            ...
            `}
            ${trackers.map((tracker) => !store_default.pending(tracker) && html`
                  <div layout="grid:2 gap:2" layout@768px="grid:2fr|2fr|3fr gap:4">
                    ${store_default.ready(tracker) && html`<ui-action>
                      <a
                    ...
                    ${!store_default.ready(tracker) && html`<ui-text type="label-m" mobile-type="label-s"> ${tracker.name} </ui-text>`}
        ...
        <div layout="row gap:5">
          ...
          <ui-button disabled="${clearedCookies}">
            <a href="${router_default.url(website_clear_cookies_default, { domain })}" data-qa="button:clear-cookies">
              Clear Cookies
            </a>
          </ui-button>
        </div>
...
//#endregion
export { website_details_default as default };

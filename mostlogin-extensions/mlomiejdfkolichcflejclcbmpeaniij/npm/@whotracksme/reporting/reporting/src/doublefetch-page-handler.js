...
import { lazyInitAsync, requireParam } from "./utils.js";
...
//#region node_modules/@whotracksme/reporting/reporting/src/doublefetch-page-handler.js
/**
* WhoTracks.Me
* https://whotracks.me/
*
...
* doublefetch.
*/
function titlesMatchAfterDoublefetch({ before: originalBefore, after: originalAfter }) {
	if (!originalBefore) {
		logger_default.warn("Pages without \"before-doublefetch\" title should have been dropped already");
		return false;
	}
	if (!originalAfter) {
		logger_default.warn("Pages without \"after-doublefetch\" title should have been dropped already");
		return false;
	}
	if (originalBefore === originalAfter) return true;
	const normalize = (x) => x.toLowerCase().replace(/\s/g, "");
	const before = normalize(originalBefore);
	const after = normalize(originalAfter);
	if (before === after) return true;
	if (before.length >= 6 && after.includes(before)) return true;
	if (after.includes("�")) {
		const nonPrintableAscii = /[^\x20-\x7f]/g;
		const brokenBefore = before.replace(nonPrintableAscii, "�");
		const start = after.replace(nonPrintableAscii, "�").indexOf(brokenBefore);
		if (start >= 0) {
			let brokenChars = 0;
			for (let i = 0; i < before.length; i += 1) {
				...
				if ((beforeChar < 32 || beforeChar > 127) && afterChar === 65533) brokenChars += 1;
				else return false;
			}
			if (brokenChars < .2 * after.length && !after.includes("���")) return true;
		}
	}
	if (before.length > after.length && after.length > 12 && after.length > .75 * before.length && isSubSequence({
		sequence: before,
		subsequence: after
	})) return true;
	return false;
...
function sanitizeActivity(activity) {
	if (!Number.isFinite(activity) || activity < 0) {
		logger_default.warn("Corrupted \"activity\" value detected:", activity, "Replacing by 0");
		return "0";
	}
	if (activity >= 1) return "1";
	if (activity <= 0) return "0";
	const noisyActivity = activity * randomBetween(.9, 1.1);
	if (noisyActivity >= 1) return "1";
	if (noisyActivity <= 0) return "0";
	return noisyActivity.toFixed(4);
}
var CachedPageFetcher = class {
	constructor() {
		this.urlToAsyncPageStructure = /* @__PURE__ */ new Map();
	}
	...
	async _doublefetchUrl(url) {
		...
		try {
			...
			if (e.isPermanentError) return { rejectReason: `unable to fetch page (${e})` };
			logger_default.debug("Failed to double-fetch", url, "(potentially it will work after a retry). Error message:", e.message);
			throw e;
		}
		if (html.length === 0) return { rejectReason: "got an empty document" };
		return { pageStructure: await analyzePageStructure(await parseHtml(html)) };
	}
};
var DoublefetchPageHandler = class {
	constructor({ jobScheduler, sanitizer, newPageApprover, pageFetcherProvider = () => new CachedPageFetcher() }) {
		...
		this.downloadLimit = 2 * 1024 * 1024;
		const config = {
			priority: -1e3,
			maxJobsTotal: 200,
		...
		jobScheduler.registerHandler("doublefetch-page", async (job) => {
			const { page } = job.args;
			const { ok, safePage, details } = await this.runJob(page);
			if (!ok) {
				logger_default.debug("Page has been rejected during doublefetch:", {
					page,
					details
				});
				return [];
	...
	async runJob(page) {
		...
		if (!preDoublefetch) throw new BadJobError("preDoublefetch missing");
		...
		const log = logger_default.debug.bind(logger_default.debug, `[doublefetch=${url}]`);
		const logWarn = logger_default.warn.bind(logger_default.warn, `[doublefetch=${url}]`);
		try {
			const checkUrl = async (urlToTest, { isCanonicalUrl }) => {
				if (await this.newPageApprover.mightBeMarkedAsPrivate(urlToTest)) return {
					ok: false,
					details: "marked as private in bloom filter"
				};
				const { pageStructure, rejectReason } = await pageFetcher.doublefetchUrl(urlToTest);
				...
				if (canonicalUrl2) {
					if (!isCanonicalUrl && urlToTest === canonicalUrl2) {
						log("Found a matching canonical URL in doublefetch result for URL:", urlToTest);
						isCanonicalUrl = true;
					} else if (urlToTest === url && canonicalUrl2 !== canonicalUrl) {
						log("Updated canonical URL from", canonicalUrl, "to", canonicalUrl2);
						try {
							const result = await checkUrl(canonicalUrl2, { isCanonicalUrl: true });
							if (result.ok) return result;
						} catch (e) {
							if (e.isPermanentError) {
								logWarn("Failed to process (corrected) canonical URL", canonicalUrl2, "(falling back to original URL now)", e);
				...
				if (!safePage) return {
					ok: false,
					details: "rejected by page sanitizer"
				};
				return {
					ok: true,
					safePage
			...
			let safePage;
			if (canonicalUrlDiffers) try {
				const result = await checkUrl(canonicalUrl, { isCanonicalUrl: true });
				if (result.ok) safePage = result.safePage;
			} catch (e) {
				if (e.isPermanentError) {
					logWarn("Failed to process canonical URL", canonicalUrl, "(falling back to original URL now)", e);
					await this._tryMarkAsPrivate(canonicalUrl);
				} else throw e;
			}
			if (!safePage) {
				const result = await checkUrl(url, { isCanonicalUrl: url === canonicalUrl });
				if (result.ok) safePage = result.safePage;
				else {
	...
	*/
	_structureMatches({ before, after, page, log }) {
		...
		if (before.noindex !== false) {
			logger_default.warn("noindex pages should have been filtered out already:", before);
			return discard("noindex must be false (before)");
		}
		if (after.noindex !== false) return discard("noindex must be false (after)");
		if (!page.title) return discard("page title missing");
		if (!before.title) return discard("title missing (before)");
		if (!after.title) return discard("title missing (after)");
		if (page.url !== before.url) if ((page.pageLoadMethod === "full-page-load" || page.title === before.title && before.title === after.title) && page.url === removeSearchHash(before.url)) log("The page url", page.url, "does not exactly match the original URL", before.url, "However, it is identical when ignoring the hash in the original URL.", "Based on the other information, it looks safe to continue.");
		else return discard(`inconsistency detected: page url does not match url (before): page.url=<<${page.url}>> !== before.url=<<${before.url}>>`);
		if (!titlesMatchAfterDoublefetch({
			before: page.title,
			after: after.title
		})) return discard(`titles do not match the page title: <<${page.title}>> ==> <<${after.title}>>`);
		if (page.pageLoadMethod !== "history-navigation" && !titlesMatchAfterDoublefetch({
			before: before.title,
			after: after.title
		})) return discard(`titles do not match the meta title: <<${before.title}>> ==> <<${after.title}>>`);
		return accept();
	}
	_sanitizePage({ page, doublefetchUrl, isCanonicalUrl, log, logWarn }) {
		const safePage = {
			...
			aggregator: { activity: sanitizeActivity(page.aggregator.activity) }
		};
		if (safePage.url !== page.url) log("url changed:", page.url, "->", safePage.url);
		if (page.search) {
			...
			if (!accept) {
				log("Omitting search query from page:", {
					query,
					reason
				});
		...
		if (isCanonicalUrl && isIndexed && safePage.requestedIndex) log("There are strong indicators that the page is public. Skipping static checks for the URL:", safePage.url);
		else if (isCanonicalUrl || isIndexed || safePage.requestedIndex) {
			const { result, reason } = sanitizeUrl(safePage.url);
			if (result !== "safe") {
				logWarn("The page appears to be public, but it will be dropped, since it failed standard static checks for the URL", safePage.url, ":", reason);
				return null;
			}
		} else {
			const { result, reason } = sanitizeUrl(safePage.url, { strict: true });
			if (result !== "safe") {
				log("Failed strict static checks for the URL", safePage.url, ":", reason);
				return null;
			}
		}
		...
		if (page.redirects && page.redirects.length > 0) safePage.redirects = page.redirects.map(({ from, to, statusCode }) => ({
			from: maskUrl(from),
			to: to === "..." ? to : maskUrl(to),
			statusCode
		}));
		if (page.ref) safePage.ref = maskUrl(page.ref);
		return safePage;
	}
...
//#endregion
export { DoublefetchPageHandler as default };

...
import { checkSameGeneralDomain, checkValidContext } from "./steps/check-context.js";
...
var RequestReporter = class {
	...
	constructor(settings, { trustedClock, countryProvider, onMessageReady, onTrackerInteraction = (event, state) => {
	...
	}, isRequestAllowed = () => false, dryRunMode = false }) {
		...
		this.pageStore = new PageStore({ notifyPageStageListeners: this.onPageStaged.bind(this) });
		this.ready = false;
		const safeOptions = [
			"extraHeaders",
			"requestBody",
	...
	*/
	...
	onPageStaged(page) {
		if (page.state === "complete" && !page.isPrivate && !page.isPrivateServer) {
			...
			if (payload.scheme.startsWith("http") && Object.keys(payload.tps).length > 0) this.telemetry({

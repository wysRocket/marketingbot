//#region node_modules/@whotracksme/reporting/reporting/src/page-structure.js
/**
* WhoTracks.Me
* https://whotracks.me/
*
...
async function analyzePageStructure(doc) {
	...
	*/
	function parseMetaRobotTags(doc) {
		...
		for (const elem of doc.querySelectorAll("html > head > meta[name=\"robots\"]")) {
			const attr = elem.getAttribute("content");
			if (attr) {
				const tags = attr.split(",").map((x) => x.trim());
				if (tags.includes("noindex") || tags.includes("none")) return {
					noindex: true,
					requestedIndex: false
				};
				requestedIndex = requestedIndex || tags.includes("all") || tags.includes("index") || tags.includes("index") || tags.includes("max-image-preview:large") || tags.includes("max-image-preview:standard") || tags.includes("max-video-preview:-1");
			}
		}
		return {
			noindex: false,
	...
	function sanitizeLanguage(lang) {
		if (lang) {
			...
			if (lang_.length <= 10) return lang_;
		}
		return null;
	}
	...
	function parseOpenGraphMetaTags(doc) {
		...
		for (const tag of [
			...
			const elem = doc.querySelector(`html > head > meta[property="og:${tag}"]`);
			if (elem) {
				const content = elem.getAttribute("content");
				if (content) og[tag] = content;
			}
		}
		return og;
	}
	...
	try {
		...
		if (noindex) return { noindex: true };
		...
		return {
			...
			meta: {
				canonicalUrl,
				language: parseHtmlLangAttribute(doc),
				contentType,
				og: parseOpenGraphMetaTags(doc)
			},
			noindex: false,
			requestedIndex
		};

...
//#region node_modules/string-natural-compare/natural-compare.js
var require_natural_compare = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var defaultAlphabetIndexMap = [];
	function isNumberCode(code) {
		return code >= 48 && code <= 57;
	}
	function naturalCompare(a, b, opts) {
		if (typeof a !== "string") throw new TypeError(`The first argument must be a string. Received type '${typeof a}'`);
		if (typeof b !== "string") throw new TypeError(`The second argument must be a string. Received type '${typeof b}'`);
		...
		if (opts) {
			...
			if (opts.alphabet) alphabetIndexMap = buildAlphabetIndexMap(opts.alphabet);
		}
		while (indexA < lengthA && indexB < lengthB) {
			...
			if (isNumberCode(charCodeA)) {
				...
				if (numStartA !== numStartB && firstDifferenceInLeadingZeros === 0) firstDifferenceInLeadingZeros = numStartA - numStartB;
			...
			if (charCodeA !== charCodeB) {
				if (charCodeA < alphabetIndexMap.length && charCodeB < alphabetIndexMap.length && alphabetIndexMap[charCodeA] !== -1 && alphabetIndexMap[charCodeB] !== -1) return alphabetIndexMap[charCodeA] - alphabetIndexMap[charCodeB];
				return charCodeA - charCodeB;
			}
			++indexA;
			++indexB;
		}
		if (indexA < lengthA) return 1;
		if (indexB < lengthB) return -1;
	...
	function buildAlphabetIndexMap(alphabet) {
		...
		if (existingMap !== void 0) return existingMap;
		const indexMap = [];
		const maxCharCode = alphabet.split("").reduce((maxCode, char) => {
			return Math.max(maxCode, char.charCodeAt(0));
		}, 0);
		for (let i = 0; i <= maxCharCode; i++) indexMap.push(-1);
		for (let i = 0; i < alphabet.length; i++) indexMap[alphabet.charCodeAt(i)] = i;
...
//#endregion
export { require_natural_compare };

var Tool = (function(){
  ...
  var getSearchQuery = function(){
    ...
    if (!$input[0]) $input = $('input[name=search_query]');
  ...
  var processPage = function(){
    ...
    if (hasCredits) {
      ...
      var html = Common.renderIframeHTML({
        ...
        settingEnabled: true,
        darkMode: isDarkMode(),
        iframeSrcParam: 'free_youtube_search_volume'
      });
      $node.html(html);
    }
    renderRankWidget({});
  ...
  var injectWidgetsRoot = function(){
    ...
    $widgetsRoot.html([
      ...
      '<div id="xt-yt-widgets-rank-container"></div>',
      ...
      '<div id="xt-yt-widgets-ltkwid-container"></div>'
    ].join('\n'));
  ...
  var injectVideoPageWidgetsRoot = function(){
    ...
    $widgetsVideoPageRoot.html([
      ...
      '<div id="xt-yt-optimize-widget"></div>',
      '<div id="xt-yt-widgets-avg-container"></div>',
      '<div id="xt-yt-widgets-tags-container"></div>'
    ].join('\n'));
  ...
  var checkWidgetsOrderTimer = null;
  
  var checkWidgetsOrder = function(){
    // Clear existing timer if any
    if (checkWidgetsOrderTimer) {
      clearTimeout(checkWidgetsOrderTimer);
    }
    
    // Set new timer for 3 seconds
    checkWidgetsOrderTimer = setTimeout(function() {
      const order = [
        '#xt-yt-freeuser-container',
        '#xt-yt-transcript-widget',
        '#xt-yt-optimize-widget',
        "#xt-yt-widgets-ltk-container",
        "#xt-yt-widgets-rank-container",
        ...
        "#xt-yt-widgets-ltkwid-container",
        ];
      const $parent = $('#xt-yt-widgets-root');
      if (!$parent.length) return;
      
      // Get current order of widgets in the parent
      const currentOrder = [];
      ...
      // Check if order needs to be corrected
      let needsReorder = false;
      const existingWidgets = [];
      
      // Collect all existing widgets in the desired order
      ...
      const currentWidgets = $parent.children().toArray();
      for (let i = 0; i < existingWidgets.length; i++) {
        if (currentWidgets[i] !== existingWidgets[i][0]) {
          needsReorder = true;
          break;
        }
      }
      ...
      if (needsReorder) {
        // Append widgets in the correct order
  ...
  var processCaptions = async function(json){
    ...
    // Render widget immediately without text
  ...
  function getVisitorDataFromPage() {
    return (
      (window.ytcfg && ytcfg.get("VISITOR_DATA")) ||
      window.INNERTUBE_CONTEXT?.client?.visitorData ||
      null
    );
  }
  ...
  async function buildYoutubeAuthorization() {
    ...
    // Prefer real SAPISID/APISID if present; otherwise fallback to 1P/3P (what your session looks like)
    const sapisidLike =
    ...
    // SAPISIDHASH MUST exist (your working request has it)
  ...
  async function getTranscriptNew(res) {
    const authorization = await buildYoutubeAuthorization();
    const visitorData = res.visitorData;
    const payload = {
      context: {
        client: {
          clientName: "WEB",
          clientVersion: res.clientVersion,
          originalUrl: `https://www.youtube.com/watch?v=${res.videoId}`,
          hl: "en",
          gl: "VN",
          visitorData,
      ...
      params: res.transcriptParam,
      externalVideoId: res.videoId,
    };

    const url = `https://www.youtube.com${res.transcriptAPIURL}`;

    try {
      const response = await fetch(url + "?prettyPrint=false", {
        method: "POST",
        mode: "cors",
        credentials: "include",
        headers: {
          ...
          "x-youtube-client-version": res.clientVersion,
          "x-origin": "https://www.youtube.com",
          "x-goog-visitor-id": visitorData,
          "x-goog-authuser": "0",
          "x-youtube-bootstrap-logged-in": "true",
          "referer": `https://www.youtube.com/watch?v=${res.videoId}`,
        },
  ...
  var renderTranscriptWidget = function(params){
    ...
    // Store title and channelName for later use
    if (params.title) transcriptTitle = params.title;
    if (params.channelName) transcriptChannelName = params.channelName;
    if (params.text) currentTranscriptText = params.text;
    
    var buttons = `
      <button class="xt-transcript-summarize-btn xt-ke-btn" data-url="https://chatgpt.com">${SVGIcons.chatgpt} ChatGPT</button>
      <button class="xt-transcript-summarize-btn xt-ke-btn" data-url="https://claude.ai/chats">${SVGIcons.claude} Claude</button>
      <button class="xt-transcript-summarize-btn xt-ke-btn" data-url="https://gemini.google.com">${SVGIcons.gemini} Gemini</button>
      <button class="xt-transcript-summarize-btn xt-ke-btn" data-url="https://chat.deepseek.com">${SVGIcons.deepseek} Deepseek</button>
    `;
    
    var iframeSrc = 'https://keywordseverywhere.com/ke/widget.php?apiKey=' + apiKey + '&source=yvsummarize&enabled=' + settingEnabled + '&pur=' + pur + '&darkmode=' + isDarkMode() + '&query=' + encodeURIComponent(Common.escapeHtml(query)) + '&country=' + settings.country + '&version=' + version;
    let html = [
      ...
      '<div class="xt-widget-iframe"><iframe src="' + iframeSrc + '" scrolling="no"></div>'
    ].join('\n');
    var selector = 'xt-yt-transcript-widget';
    var $root = $('#' + selector);
    var existingIframe = $root.find('iframe')[0];
    var existingSrc = existingIframe ? existingIframe.getAttribute('src') : '';
    if (existingIframe && existingSrc === iframeSrc && $root.data('xt-rendered')) {
      return;
    }
    $root.html(html);
    $root.data('xt-rendered', true);
    ...
    $root.find('.xt-transcript-summarize-btn').click(function(e){
      ...
      // Ensure we have title and channel name (get from page if not set)
      if (!transcriptTitle) transcriptTitle = getVideoTitleFromPage();
      if (!transcriptChannelName) transcriptChannelName = getChannelNameFromPage();
      
      $('[visibility="ENGAGEMENT_PANEL_VISIBILITY_EXPANDED"]').css('order', 1);
      // If transcript is not available, trigger button click to get it
      if (!currentTranscriptText) {
        // Set up callback to send message once transcript is received
        transcriptButtonClickCallback = function() {
          chrome.runtime.sendMessage({
            ...
            data: {
              ...
              channelName: transcriptChannelName
            }
          }, function(){});
        };
        
        // Trigger the transcript button click
        getTranscriptByButtonClick(transcriptTitle, transcriptChannelName);
      } else {
        // Transcript already available, send message immediately
        chrome.runtime.sendMessage({
          cmd: 'yt.transcript.summarize',
          data: {
            url: buttonUrl,
  ...
  var renderOptimizeWidget = function(res){
    var settings = Starter.getSettings();
    var apiKey = settings.apiKey || '';
    var settingEnabled = settings.sourceList.youtag;
    var query = getSearchQuery();
    ...
    var version = chrome.runtime.getManifest().version;
    var buttons = `
      <button class="xt-optimize-btn xt-ke-btn" data-url="https://chatgpt.com">${SVGIcons.chatgpt} ChatGPT</button>
      <button class="xt-optimize-btn xt-ke-btn" data-url="https://claude.ai/chats">${SVGIcons.claude} Claude</button>
      <button class="xt-optimize-btn xt-ke-btn" data-url="https://gemini.google.com">${SVGIcons.gemini} Gemini</button>
      <button class="xt-optimize-btn xt-ke-btn" data-url="https://chat.deepseek.com">${SVGIcons.deepseek} Deepseek</button>
    `;
    let html = [
      '<div class="xt-close">✖</div>',
      `<h3><img src="${chrome.runtime.getURL('img/icon64.png')}"> Optimize This Video</h3>`,
      `<div class="xt-optimize-buttons">
        ${buttons}
      </div>`,
      `<div id="xt-optimize-wait-message" class="xt-hidden">Please wait for the Search Insights widget to finish computing all the data.</div>`,
      '<div class="xt-widget-iframe"><iframe src="https://keywordseverywhere.com/ke/widget.php?apiKey=' + apiKey + '&source=yvoptimize&enabled=' + settingEnabled + '&pur=' + pur + '&darkmode=' + isDarkMode() + '&query=' + encodeURIComponent(Common.escapeHtml(query)) + '&country=' + settings.country + '&version=' + version + '" scrolling="no"></div>'
    ].join('\n');
    var selector = 'xt-yt-optimize-widget';
    var $root = $('#' + selector);
    $root.html(html);
    $root.addClass('xt-ke-card');
    checkWidgetsOrder();
    ...
    $root.find('.xt-optimize-btn').click(async function(e){
      // if (advancedMetricsLoading) {
      //   $('#xt-rank-wait-message').removeClass('xt-hidden');
      //   return;
      // }
      const url = this.dataset.url;
      const llmSource = getLLMSourceFromUrl(url);
      const template = await getOptimizeTemplate(llmSource);
      // console.log(res);
      // console.log(template);
      // console.log(rankTemplateData);
      rankTemplateData.search_query = getSearchQuery();
      rankTemplateData.optimization_score = res.optimizationScore;
      rankTemplateData.optimization_score_level =
        res.optimizationScore >= 90 ? 'high' :
        res.optimizationScore >= 75 ? 'medium' :
        res.optimizationScore >= 0 ? 'low' : '';
      rankTemplateData.engagement_score = res.engagementScore;
      rankTemplateData.engagement_score_level =
        res.engagementScore >= 6 ? 'high' :
        res.engagementScore >= 3 ? 'medium' :
        res.engagementScore >= 0 ? 'low' : '';
      rankTemplateData.views_per_day = parseInt(res.viewsPerDay.replace(/,/g, ''));
      rankTemplateData.views_per_day_level =
        rankTemplateData.views_per_day >= 6 ? 'high' :
        rankTemplateData.views_per_day >= 3 ? 'medium' :
        rankTemplateData.views_per_day >= 0 ? 'low' : '';
      rankTemplateData.topic_expertise = res.expertisePoints;
      rankTemplateData.topic_expertise_level =
        res.expertisePoints >= 10 ? 'high' :
        res.expertisePoints >= 5 ? 'medium' :
        res.expertisePoints >= 0 ? 'low' : '';
      rankTemplateData.total_subscriber_count = res.subscribers;
      rankTemplateData.total_subscriber_count_level = getSubscriberLevel(rankTemplateData.total_subscriber_count);
      rankTemplateData.broad_match_in_title = res.keywordOptimization?.broadMatchesTitle;
      rankTemplateData.broad_match_in_title_level = res.keywordOptimization?.broadMatchesTitle >= 10 ? 'high' : 'low';
      rankTemplateData.broad_match_in_description = res.keywordOptimization?.broadMatchesDescr;
      rankTemplateData.broad_match_in_description_level = res.keywordOptimization?.broadMatchesDescr >= 10 ? 'high' : 'low';
      rankTemplateData.exact_match_in_title = res.keywordOptimization?.exactMatchesTitle;
      rankTemplateData.exact_match_in_title_level = res.keywordOptimization?.exactMatchesTitle >= 10 ? 'high' : 'low';
      rankTemplateData.exact_match_in_description = res.keywordOptimization?.exactMatchesDescr;
      rankTemplateData.exact_match_in_description_level = res.keywordOptimization?.exactMatchesDescr >= 10 ? 'high' : 'low';
      rankTemplateData.total_tag_characters = res.tagsCharsLenPoints;
      rankTemplateData.total_tag_characters_level =
        res.tagsCharsLenPoints > 6 ? 'high' :
        res.tagsCharsLenPoints >= 4 ? 'medium' :
        'low';
      rankTemplateData.total_tags = res.tagsLenPoints;
      rankTemplateData.total_tags_level =
        res.tagsLenPoints > 6 ? 'high' :
        res.tagsLenPoints >= 4 ? 'medium' :
        'low';

      const prompt = generatePrompt(template.prompt, rankTemplateData);
      chrome.runtime.sendMessage({
  ...
  var getVideoInsights = async function(html){
    ...
    if (!tags.length) rankTemplateData.all_tags = "The video does not have any tags";
    getTagsMetrics(tags);
    ...
    if (likes + dislikes > 0) {
      ...
      engagementScore = parseFloat(((likes + dislikes + commentsCount) * 100 / viewCount).toFixed(2));
    }
      ...
      function(json){
        ...
        if (json && json.data && Object.keys(json.data).length) {
          ...
          Object.values(json.data).map(item => {
            res.push(item.keyword + ', ' + item.vol);
          })
          rankTemplateData.all_tags = res.join('\n');
        }
      }
    );
  }
  ...
  var processYTKeywordsResponse = function( json, keywords, usage ){
    ...
    let mostUsedTags = [];
    widgetRows.map(item => {
      mostUsedTags.push(item.keyword + ', ' + item.vol);
    });
    rankTemplateData.most_used_video_tags = mostUsedTags.join('\n');
  ...
  var initWindowMessaging = function(){
    window.addEventListener("message", function(event){
      ...
      if (payload.cmd === 'xt-get-config') {
        ...
        else if (source === 'ytltkwid') selector = '#xt-yt-ltkwid .xt-widget-iframe';
        else if (source === 'ytrank') selector = '#xt-yt-widgets-rank-container .xt-widget-iframe';
        else if (source === 'also') selector = '#xt-yt-also .xt-widget-iframe';
        else if (source === 'free_youtube_search_volume') selector = '.xt-youtube-query .xt-widget-iframe';
        ...
        else if (source === 'yvoptimize') selector = '#xt-yt-optimize-widget';
        if (!selector) return;
        if (height <= 0) return;
  ...
  var transcriptRequestPending = false;
  var transcriptTitle = '';
  var transcriptChannelName = '';
  var currentTranscriptText = '';
  var transcriptButtonClickCallback = null;

  var initTranscriptListener = function(){
    // Listen for intercepted transcript responses via custom event
    window.addEventListener('xt-youtube-transcript', function(event) {
      if (!transcriptRequestPending) return;
      
      const data = event.detail.data;
      const transcriptText = extractTranscript(data);
      
      if (transcriptText) {
        currentTranscriptText = transcriptText.replace(/&#39;/g, "'");
        renderTranscriptWidget({
          text: currentTranscriptText,
          title: transcriptTitle,
          channelName: transcriptChannelName
        ...
        // If there's a pending callback (button was clicked), execute it now
        if (transcriptButtonClickCallback) {
          transcriptButtonClickCallback();
          transcriptButtonClickCallback = null;
        }
      }
    });

    // Also check for template elements (fallback method)
    var checkTranscriptTemplates = setInterval(function(){
      var templates = document.querySelectorAll('template[data-type="transcript"]');
      if (templates.length > 0 && transcriptRequestPending) {
        for (var i = 0; i < templates.length; i++) {
          var template = templates[i];
          try {
            var json = JSON.parse(template.textContent);
            var transcriptText = extractTranscript(json);
            if (transcriptText) {
              currentTranscriptText = transcriptText.replace(/&#39;/g, "'");
              renderTranscriptWidget({
                text: currentTranscriptText,
  ...
  var getVideoTitleFromPage = function(){
    // Try multiple selectors to get video title
    var title = '';
    var titleEl = document.querySelector('h1.ytd-watch-metadata yt-formatted-string, h1.ytd-watch-metadata, #title h1 yt-formatted-string, ytd-watch-metadata h1');
    if (titleEl) {
  ...
  var getChannelNameFromPage = function(){
    // Try multiple selectors to get channel name
    var channelName = $('.ytd-channel-name #text').attr('title');
    return channelName.trim();
  };

  var getTranscriptByButtonClick = function(title, channelName){
    // If title/channelName not provided, try to get from page DOM
    if (!title) title = getVideoTitleFromPage();
    if (!channelName) channelName = getChannelNameFromPage();
    
    transcriptTitle = title;
    transcriptChannelName = channelName;
    transcriptRequestPending = true;

    // Find the "Show transcript" button using the exact selector pattern from the user
    var button = document.querySelector('button[aria-label="Show transcript"]');

    if (button) {
      button.click();
    } else {
      // Button not found, try waiting a bit for it to appear
      setTimeout(function(){
        if (transcriptRequestPending) {
          getTranscriptByButtonClick(title, channelName);
        }
      }, 1000);
      // If still not found after 3 seconds, show empty transcript
      setTimeout(function(){
        if (transcriptRequestPending) {
          transcriptRequestPending = false;
  ...
  var processQueryResponse = function( json ){
    ...
    if (!data) {
      ...
      if(data.vol != '-') {
        rankTemplateData.search_volume = data.vol;
  ...
  var runVideosProcessing = function(){
    ...
    processShorts($('ytm-shorts-lockup-view-model'));
    var settings = Starter.getSettings();
  ...
  var processShorts = function(children){
    for (var i = 0, len = children.length; i < len; i++) {
      if (i >= 4) return;
      var node = children[i];
      processShort(node);
  ...
  var processShort = function(node){
    const $node = $(node);
    const res = {};
    res.url = $node.find('a')[0].href;
    res.title = $node.find('h3[aria-label]').attr('aria-label');
    shortsCache.cache[res.url] = res;
    shortsCache.order.push(res);
  };
  ...
  var getVideoData = function($node){
    ...
    var ageStr = '';
    var duration;
    var durationBadge = $node.find('.ytd-thumbnail-overlay-time-status-renderer .badge-shape-wiz__text')[0];
    if (durationBadge) {
      var pair = durationBadge.textContent.split(':');
      duration = parseInt(pair[0]) * 60 + parseInt(pair[1]);
    }
  ...
  var calcAvg = function(params){
    ...
    let averageDuration = durationCount > 0 ? totalDuration / durationCount : 0;
    ...
    var percentAddedIn6Weeks = Math.round(sumAddedIn6Weeks / count * 100);

    rankTemplateData.ranking_difficulty = difficultyAvg;
    rankTemplateData.ranking_difficulty_level = difficultyAvg >= 70
      ? 'hard' : (difficultyAvg >= 50
      ? 'medium' : (difficultyAvg >= 30
      ? 'easy' : (difficultyAvg
      ? 'veryeasy' : '')));
    rankTemplateData.top_channel_name = topCreatorShort;
    rankTemplateData.top_channel_videos = topCreatorVideos;
    rankTemplateData.maximum_views = maxViews;
    rankTemplateData.average_views = avgViews;
    rankTemplateData.average_views_level =
      avgViews >= 10000 ? 'high' :
      avgViews >= 1000 ? 'medium' :
      avgViews >= 0 ? 'low' : '';

    if (avgYears > 0) {
      rankTemplateData.average_age = avgYears + ' year' + (avgYears > 1 ? 's' : '');
    }
    else {
      rankTemplateData.average_age = avgDays + ' days';
    }
    rankTemplateData.average_age_level =
      avgDays >= 365 * 2 ? 'high' :
      avgDays >= 365 / 2 ? 'medium' :
      avgDays >= 0 ? 'low' : '';

    rankTemplateData.added_last_6weeks_percent = percentAddedIn6Weeks;
    rankTemplateData.added_last_6weeks_percent_level =
      percentAddedIn6Weeks >= 50 ? 'high' :
      percentAddedIn6Weeks >= 10 ? 'medium' :
      percentAddedIn6Weeks >= 0 ? 'low' : '';

    rankTemplateData.keywords_in_title_percent = percentInTitle;
    rankTemplateData.keywords_in_title_percent_level =
      percentInTitle >= 70 ? 'high' :
      percentInTitle >= 20 ? 'medium' :
      percentInTitle > 0 ? 'low' :
      percentInTitle === 0 ? 'zero' : '';

    rankTemplateData.keywords_in_desc_percent = percentInDsc;
    rankTemplateData.keywords_in_desc_percent_level =
      percentInDsc >= 70 ? 'high' :
      percentInDsc >= 20 ? 'medium' :
      percentInDsc >= 0 ? 'low' : '';

    rankTemplateData.total_verified_channels_percent = percentTotalVerified;
    rankTemplateData.total_verified_channels_percent_level =
      percentTotalVerified >= 70 ? 'high' :
      percentTotalVerified >= 30 ? 'medium' :
      percentTotalVerified >= 0 ? 'low' : '';

    rankTemplateData.average_video_length = averageDuration;
    rankTemplateData.average_video_length_level =
      averageDuration >= 60*60 ? 'veryhigh' :
      averageDuration >= 15*60 ? 'high' :
      averageDuration >= 5*60 ? 'medium' :
      averageDuration >= 2*60 ? 'low' :
      averageDuration >= 0 ? 'verylow' : '';

    rankTemplateData.maximum_video_length = maxDuration;
    rankTemplateData.maximum_video_length_level =
      maxDuration >= 60*60 ? 'veryhigh' :
      maxDuration >= 15*60 ? 'high' :
      maxDuration >= 5*60 ? 'medium' :
      maxDuration >= 2*60 ? 'low' :
      maxDuration >= 0 ? 'verylow' : '';

    rankTemplateData.do_shorts_exist = shortsCache.order.length > 0 ? 'yes' : 'no';
    rankTemplateData.top_short_videos = shortsCache.order.map(function(item){
      return `
        Shorts Video URL: ${item.url}
        Shorts Video Title: ${item.title}
      `;
    }).join('\n');
    ...
    renderAvg({
      ...
      percentAddedIn6Weeks: percentAddedIn6Weeks,
  ...
  const getSubscriberLevel = (count) => {
    if (count > 1000000) return "veryhigh";
    if (count >= 100000 && count <= 1000000) return "high";
    if (count >= 10000 && count < 100000) return "medium";
    if (count >= 1000 && count < 10000) return "low";
    return "verylow";
  };


  var processAdvancedData = function() {
    const topSerpVideos = [];
    let minimumSubscriberCount = Infinity;
    let maximumSubscriberCount = -Infinity;
    const descriptionLength = 1000;

    videoCache.order.map((href, index) => {
      if (index >= 6) return;
      const item = videoCache.cache[href];
      const subs = item.advanced?.subscribers || 0;
      let description = (item.advanced?.description || '').substr(0, descriptionLength);
      ...
      `);
      if (subs > 0) {
        if (subs < minimumSubscriberCount) minimumSubscriberCount = subs;
        if (subs > maximumSubscriberCount) maximumSubscriberCount = subs;
      }
    });

    rankTemplateData.top_serp_videos = topSerpVideos.join('\n');
    rankTemplateData.minimum_subscriber_count = minimumSubscriberCount === Infinity ? 0 : minimumSubscriberCount;
    rankTemplateData.maximum_subscriber_count = maximumSubscriberCount === -Infinity ? 0 : maximumSubscriberCount;
    rankTemplateData.minimum_subscriber_count_level = getSubscriberLevel(rankTemplateData.minimum_subscriber_count);
    rankTemplateData.maximum_subscriber_count_level = getSubscriberLevel(rankTemplateData.maximum_subscriber_count);
  };
  ...
  var getAvgHtml = async function(data) {
    ...
    var metrics;
    var volume;
    if (data.metricsPromise) {
      metrics = await data.metricsPromise;
      try {
  ...
  var renderRankWidget = function(params){
    var settings = Starter.getSettings();
    var apiKey = settings.apiKey || '';
    var settingEnabled = settings.sourceList.youtag;
    var query = getSearchQuery();
    ...
    var version = chrome.runtime.getManifest().version;
    var buttons = `
      <button class="xt-rank-summarize-btn xt-ke-btn" data-url="https://chatgpt.com">${SVGIcons.chatgpt} ChatGPT</button>
      <button class="xt-rank-summarize-btn xt-ke-btn" data-url="https://claude.ai/chats">${SVGIcons.claude} Claude</button>
      <button class="xt-rank-summarize-btn xt-ke-btn" data-url="https://gemini.google.com">${SVGIcons.gemini} Gemini</button>
      <button class="xt-rank-summarize-btn xt-ke-btn" data-url="https://chat.deepseek.com">${SVGIcons.deepseek} Deepseek</button>
    `;
    let html = [
      '<div class="xt-close">✖</div>',
      `<h3><img src="${chrome.runtime.getURL('img/icon64.png')}"> How can I rank for this YouTube search</h3>`,
      `<div class="xt-rank-summarize-buttons">
        ${buttons}
      </div>`,
      `<div id="xt-rank-wait-message" class="xt-hidden">Please wait for the Search Insights widget to finish computing all the data.</div>`,
      '<div class="xt-widget-iframe"><iframe src="https://keywordseverywhere.com/ke/widget.php?apiKey=' + apiKey + '&source=ytrank&enabled=' + settingEnabled + '&pur=' + pur + '&darkmode=' + isDarkMode() + '&query=' + encodeURIComponent(Common.escapeHtml(query)) + '&country=' + settings.country + '&version=' + version + '" scrolling="no"></div>'
    ].join('\n');
    var selector = 'xt-yt-widgets-rank-container';
    var $root = $('#' + selector);
    $root.html(html);
    $root.addClass('xt-ke-card');
    checkWidgetsOrder();
    ...
    $root.find('.xt-rank-summarize-btn').click(async function(e){
      if (advancedMetricsLoading) {
        $('#xt-rank-wait-message').removeClass('xt-hidden');
        return;
      }
      const url = this.dataset.url;
      const llmSource = getLLMSourceFromUrl(url);
      const template = await getRankTemplate(llmSource);
      rankTemplateData.search_query = getSearchQuery();
      const prompt = generatePrompt(template.prompt, rankTemplateData);
      chrome.runtime.sendMessage({
        cmd: 'prompt_pending.set',
  ...
  var autocompleteQuery = function(query){
  ...
  var getRankTemplate = function(llmSource){
    return new Promise((resolve) => {
      chrome.runtime.sendMessage({
        cmd: 'api.openAIFetchKEPrompt',
        data: {id: 'youtubesearchoptimizer', source: llmSource || ''}
      }, function(response){
        resolve(response);
      });
    })
  ...
  var getOptimizeTemplate = function(llmSource){
    return new Promise((resolve) => {
      chrome.runtime.sendMessage({
        cmd: 'api.openAIFetchKEPrompt',
        data: {id: 'youtubevideooptimizer', source: llmSource || ''}
      }, function(response){
        resolve(response);
      });
    })
  ...
  var renderWidgetTable = function(type, rows, json, nocredits){
    ...
    if (type === 'ltkwid') {
      ...
      iframeSrcParam = 'ytltkwid';
      rootSelector = 'xt-yt-ltkwid';
      addToSelector = '#xt-yt-widgets-ltkwid-container';
    }
    var settings = Starter.getSettings();
    var params = {
  ...
  var getAdvancedMetrics = function(){
    ...
    Promise.all(promises).then(function(items){
      ...
      document.body.setAttribute('xt-advanced-metrics', true);
    });
  };
  ...
  var addSERPItemMetrics = function(root, data){
    ...
    if (likes + dislikes > 0) {
      approvalScore = Math.round(likes * 100 / (likes + dislikes));
      engagementScore = parseFloat(((likes + dislikes + commentsCount) * 100 / (data.viewCount)).toFixed(2));
  ...
  var getVideoInfo = async function(dom, html, params){
    let res = {};
    res.videoId = dom.querySelector('meta[itemprop=identifier]').getAttribute('content');
    ...
    let promises = $('script', dom).map(async function(i, script){
      ...
      if (text.indexOf('var ytInitialData = ') !== -1) {
        ...
        try {
          ...
          let panels = getObjValue(json, '.engagementPanels');
          let transcriptAPIURL, transcriptParam;

          // Render widget immediately without text
          if (panels && Array.isArray(panels)) {
            for (let panel of panels) {
              const panelId = getObjValue(panel, '.engagementPanelSectionListRenderer.panelIdentifier');
              if (panelId === 'engagement-panel-searchable-transcript') {
                ...
                break; // Stop once the desired panel is found
              }
            }
          }
          ...
          if (!continuation) {
            continuation = getObjValue(json, '.contents.twoColumnWatchNextResults.results.results.contents[4].itemSectionRenderer.contents[0].continuationItemRenderer.continuationEndpoint.continuationCommand.token');
          }
          if (!continuation) {
    ...
    if (res.channelId) {
      ...
      if (params.getTotalVideos && res.channelId) {
  ...
  var getChannelTotalVideos = async function(res){
    ...
    console.log(info);
    let channelDataURL = `https://www.youtube.com/youtubei/v1/browse?prettyPrint=false`;
    let payload = {
        "context":
        {
            "client":
  ...
  var processYtInitial = function(res){
    ...
    try {
      ...
      if (!likes) likes = getObjValue(videoPrimaryInfoRenderer, '.videoActions.menuRenderer.topLevelButtons[0].segmentedLikeDislikeButtonViewModel.likeButtonViewModel.likeButtonViewModel.toggleButtonViewModel.toggleButtonViewModel.toggledButtonViewModel.buttonViewModel.title');
      if (!likes) {
        for (var i = 0; i < 6; i++) {
      ...
      if (res.description.toLowerCase().indexOf(queryLC) !== -1) res.descriptionHasQuery = true;
      if (res.title) rankTemplateData.video_title = res.title;
  ...
  var calcExpertisePoints = function(channelId, ytInitialData){
    ...
    results.map(result => {
      let id = getObjValue(result, '.lockupViewModel.metadata.lockupMetadataViewModel.image.decoratedAvatarViewModel.rendererContext.commandContext.onTap.innertubeCommand.browseEndpoint.browseId');
      let title = getObjValue(result, '.compactVideoRenderer.title.simpleText');
      if (!id) return;
      total++;

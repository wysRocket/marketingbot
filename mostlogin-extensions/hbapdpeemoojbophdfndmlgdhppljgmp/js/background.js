  ...
  var initMessaging = function(){
    chrome.runtime.onMessage.addListener(
      function(request, sender, sendResponse) {
        ...
        if (cmd === 'api.isOnline') {
        ...
        else if (cmd === 'api.getCompetitorUrlKeywords') {
          API.getCompetitorUrlKeywords(data, function(json){
            sendResponse(json);
          });
          return true;
        }
        ...
        else if (cmd === 'api.getMatchingUrls') {
          API.getMatchingUrls(data, function(json){
            sendResponse(json);
          });
          return true;
        }
          ...
          API.openAIFetchSEOReports(source).then(function(json){
            sendResponse(json);
          });
          return true;
        }
          ...
          const source = data.source || '';
          API.openAIFetchSEOReport(data.id, source).then(function(json){
            sendResponse(json);
          });
          return true;
        }
        ...
        else if (cmd === 'api.openAIFetchKEPrompt') {
          const source = data.source || '';
          API.openAIFetchKEPrompt(data.id, source).then(function(json){
            sendResponse(json);
          });
          return true;
        }
        ...
        else if (cmd === 'google.setAIOData') {
          googleAioData = data;
        }
        else if (cmd === 'google.getAIOData') {

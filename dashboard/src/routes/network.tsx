          ...
          <div style={{ padding: 12, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {[...dd.endpoints.entries()].sort((a, b) => b[1].count - a[1].count).slice(0, 15).map(([k, v]) => (
              <div key={k} style={{ background: '#181c24', padding: '4px 8px', borderRadius: 4, fontSize: 10, border: '1px solid #2a2f3a' }}>
                ...
                <span style={{ color: '#8b949e', marginLeft: 4 }}>×{v.count}</span>
                {v.body && <div style={{ marginTop: 2, color: '#8b949e', maxWidth: 250, overflow: 'hidden', textOverflow: 'ellipsis' }} title={v.body}>{v.body.slice(0, 60)}</div>}
              </div>
            ))}
          </div>
        </div>
      ))}
      {domainMap.length === 0 && <div style={{ gridColumn: 'span 12', textAlign: 'center', color: '#8b949e', padding: 40 }}>No extension network events</div>}
    </div>
  )
}

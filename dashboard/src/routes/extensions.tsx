  ...
  return (
    <div style={{ background: '#11151c', border: '1px solid #2a2f3a', borderRadius: 6 }}>
      <div style={{ padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        ...
        <label style={{ fontSize: 10, color: '#8b949e', textTransform: 'uppercase' }}>Method</label>
        ...
        <span style={{ marginLeft: 'auto', fontSize: 11, color: '#8b949e' }}>{filtered.length} events</span>
      </div>
        ...
        <tbody>
          {filtered.slice(0, 500).map((e, i) => (
            <tr key={i} style={{ borderBottom: '1px solid rgba(42,47,58,.4)' }}>
              ...
              <td style={{ padding: '6px 10px', maxWidth: 100, overflow: 'hidden', textOverflow: 'ellipsis', fontFamily: 'JetBrains Mono, monospace', fontSize: 10, color: '#8b949e' }}>{(e.requestBody || '—').slice(0, 60)}</td>
              <td style={{ padding: '6px 10px', maxWidth: 100, overflow: 'hidden', textOverflow: 'ellipsis', fontFamily: 'JetBrains Mono, monospace', fontSize: 10, color: '#8b949e' }}>{(e.responseBody || '—').slice(0, 60)}</td>
            </tr>
          ))}
        </tbody>
      </table>

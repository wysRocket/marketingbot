...
function handleResponse(event: any): void {
  ...
  (async () => {
    ...
    try {
      const bodyResult = await cdpRef.send("Network.getResponseBody", { requestId }) as any;
      const raw = bodyResult?.body ?? null;
      if (raw) {
        responseBody = raw.length > 10000 ? raw.slice(0, 10000) + "\n… [truncated]" : raw;
        if (responseBody) {
    ...
    const obs: Observation = {
      ...
      timing: { requestAt: meta.requestAt, responseAt: Date.now(), durationMs: Date.now() - meta.requestAt },
    };
    observations.push(obs);
    await fs.mkdir(path.dirname(OUT), { recursive: true });
    await fs.appendFile(OUT, JSON.stringify(obs) + "\n", "utf8");
    const short = meta.url.length > 70 ? meta.url.slice(0, 70) + "…" : meta.url;
    console.log(`[${meta.matchedDomain}] ${meta.method} ${response?.status ?? "?"} ${short}`);
  })();
...
async function run(): Promise<void> {
  ...
  const links = await page.$$eval("a[href]", (as: HTMLAnchorElement[]) =>
    as.slice(0, 8).map((a) => a.href).filter((h: string) => h.startsWith("http"))
  );
  for (const link of links.slice(0, 3)) {
    try { await page.goto(link, { waitUntil: "domcontentloaded", timeout: 8000 }); await page.waitForTimeout(3000); } catch {}
  }
  ...
  for (const o of observations) { byDomain[o.matchedDomain] = (byDomain[o.matchedDomain] || 0) + 1; }
